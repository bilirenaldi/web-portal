...
$excel->getProperties()->setCreator('Himsif')
					   ->setLastModifiedBy('Himsif')
					   ->setTitle("Data Absensi")
					   ->setSubject("Absen")
					   ->setDescription("Laporan Data Absen")
...
$cid = $_GET['aid'];
$query = mysqli_query($con,"SELECT keterangan from absen where id='$cid'");
$row  = mysqli_fetch_array($query);

$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA ABSEN $row[keterangan]"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$cid = $_GET['aid'];
$kom= mysqli_query($con,"SELECT COUNT(*) as total from tblhadir,tblusers where tblhadir.user_id = tblusers.id and tblhadir.absen_id = '$cid'");
$data=mysqli_fetch_assoc($kom);

$excel->setActiveSheetIndex(0)->setCellValue('A2', "JUMLAH ANGGOTA HADIR : $data[total]");
...
$excel->setActiveSheetIndex(0)->setCellValue('E3', "JABATAN"); // Set kolom E3 dengan tulisan "TELEPON"
...
while($data = mysqli_fetch_array($query)){ // Ambil semua data dari hasil eksekusi $sql
	...
	$excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $data['type']);
...
$excel->getActiveSheet(0)->setTitle("Laporan Data ABSEN");
...
header("Content-Disposition: attachment; filename=Data Absen $row[keterangan].xlsx"); // Set nama file excel nya
